...
if [ "$MODE" = "unittest" ]; then
  EXP_NAME=${MODEL}_left_right_test
...
TRAIN_FILE_LIST=$CODE_PATH/file_list/left_right_train_files.txt
TEST_FILE_LIST=$CODE_PATH/file_list/left_right_val_files.txt

# Train
CUDA_VISIBLE_DEVICES=1 python $CODE_PATH/main.py --train \
  --exp_name=$EXP_NAME --data=left_right --model=$MODEL --train_file_list=$TRAIN_FILE_LIST \
  --test_file_list=$TEST_FILE_LIST --batch_size=$BATCH_SIZE \
  --image_height=$IMAGE_HEIGHT --image_width=$IMAGE_WIDTH \
  --output_height=$OUTPUT_HEIGHT --output_width=$OUTPUT_WIDTH \
  --num_scale=$NUM_SCALE --num_class=$NUM_CLASS --image_channel=6 \
...
#   --exp_name=$EXP_NAME --data=left_right --model=$MODEL \
#   --image_height=$IMAGE_HEIGHT --image_width=$IMAGE_WIDTH \
#   --output_height=$OUTPUT_HEIGHT --output_width=$OUTPUT_WIDTH \
#   --num_scale=$NUM_SCALE --num_class=$NUM_CLASS --image_channel=6 \
#   --init_model_path=$MODEL_PATH \
