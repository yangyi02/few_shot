...
class Base3DFNet(nn.Module):
    def __init__(self, im_channel, dp_channel, fl_channel, num_class):
        super(Base3DFNet, self).__init__()
        num_hidden = 16
        self.bn0 = nn.BatchNorm2d(im_channel + dp_channel + fl_channel)
        self.conv1 = nn.Conv2d(im_channel + dp_channel + fl_channel, num_hidden, 3, 1, 1)
    ...
    def forward(self, images, depths, flows):
        for i in range(len(images)):
            x0 = torch.cat((images[i], depths[i], flows[i]), 1)
            x0 = self.bn0(x0)
