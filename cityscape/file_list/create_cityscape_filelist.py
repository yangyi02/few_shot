...
disparity_name= 'disparity'
flow_name = 'flow_unsup'
...
for status in status_list:
    output_file = 'cityscape_' + status + '_files.txt'
    ...
    flow_folder = os.path.join(flow_dir, status)
    ...
    for folder in folders:
        ...
        flo_folder = os.path.join(flow_folder, folder)
        flo_file_names = os.listdir(flo_folder)
        flo_file_names = sorted(flo_file_names)
        ...
        for i in range(len(segme_file_names)):
                if segme_file_names[i].endswith('_gtFine_labelIds.png'):
                    ...
                    fl = os.path.join(flo_folder, file_id + '_flow.png')
