...
2. Go to `file_list` folder, open `create_cityscape_filelist.py` and change `cityscape_dir` to your own cityscape data path, and run `python create_cityscape_filelist.py`.
...
- To obtain depth and flow, we use ground truth existed in the some datasets (i.e. [MLT](http://robots.princeton.edu/projects/2016/PBRS/), [VDrift](http://www.cvlibs.net/datasets/kitti/)). For other real world datasets (i.e. [KITTI](http://www.cvlibs.net/datasets/kitti/), [CityScape](https://www.cityscapes-dataset.com/)), we use the unperfect unsupervisedly learned depth and optical flow estimation from [Joint Unsupervised Learning of Optical Flow and Depth by Watching Stereo Videos](https://arxiv.org/abs/1810.03654).
