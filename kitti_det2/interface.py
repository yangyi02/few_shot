...
class DetectInterface(object):
    def __init__(self, train_dataloader, test_dataloader, model, learning_rate,
                 train_iteration, test_interval, test_iteration, save_interval,
    ...
    def train(self):
        ...
        for epoch in range(self.train_epoch):
            for _, sample in enumerate(self.train_dataloader):
                im, dp, fl = sample['images'], sample['depths'], sample['flows']
                hm, of = sample['heatmaps'], sample['offsets']

                optimizer.zero_grad()
                pred, pred_of = self.model(im, dp, fl)
                loss = 0
                for i in range(len(hm)):
                    loss = loss + criterion_conf(pred[i], hm[i])
                loss = loss * 10
                for i in range(len(hm)):
                    mask = hm[i] > 0
                    mask = mask.float()
                ...
                logging.info('epoch %d, iteration %d, train loss: %.2f, average train loss: %.2f',
                             epoch, it, loss, ave_train_loss)
                ...
                if (it + 1) % self.test_interval == 0:
                    ...
                    self.model.train()
                    torch.set_grad_enabled(True)
            logging.info('finish epoch %d, saving model', epoch)
            with open(self.save_model_path, 'w') as handle:
                torch.save(self.model.state_dict(), handle)
        writer.close()

    def validate(self):
        test_loss = self.test(self.test_iteration)
    ...
    def test(self, test_iteration=None):
        self.model.eval()
        torch.set_grad_enabled(False)
        criterion_conf = detect_loss.BCELoss2d()
        criterion_loc = detect_loss.MSELoss2d()
        test_loss_all, test_acc_all = [], []
        for it, sample in enumerate(self.test_dataloader):
            im, dp, fl = sample['images'], sample['depths'], sample['flows']
            hm, of = sample['heatmaps'], sample['offsets']
            ...
            logging.info('at iteration %d, test loss: %.2f', it, loss)
            if test_iteration:
                if it >= self.test_iteration:
                    break
